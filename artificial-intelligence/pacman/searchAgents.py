...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        # if current state is in a corner
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            visited = state[1]  # corners visited
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    totalCost = 0  # start at 0 (default)
    node = state[0]  # current position
    visited = state[1]  # corners visited

    # Initialize not visited list
    notVisited = []  # corners not visited
    ...
    while len(notVisited) > 0:
        # Find the min cost to a corner
        minCost = 999999  # default to a large number
        minCorner = None  # default to None

        for corner in notVisited:
            cost = util.manhattanDistance(node, corner)  # cost to corner
            if cost < minCost:
                minCost = cost
                minCorner = corner

        heuristicCost, corner = minCost, minCorner  # heuristic cost to corner

        notVisited.remove(corner)
        totalCost += heuristicCost  # add cost to total
        node = corner  # Change the current position to the corner
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    # Use problem's mazeDistance function to calculate the actual shortest path
    def mazeDistance(point1, point2):
        # Shortest path from point1 to point2
        return len(
            search.bfs(
                PositionSearchProblem(
                    problem.startingGameState, start=point1, goal=point2, warn=False
                )
            )
        )

    # Find the farthest food from the current position
    farthestDistance = max(mazeDistance(position, food) for food in foodList)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        # JIMMY HARVIN, PHILIP WALLIS
        # JAH200003, PTW190000
        return search.bfs(problem)  # return the path to the closest dot via bfs
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        # JIMMY HARVIN, PHILIP WALLIS
        # JAH200003, PTW190000
        return self.food[x][y]  # if food, return true
